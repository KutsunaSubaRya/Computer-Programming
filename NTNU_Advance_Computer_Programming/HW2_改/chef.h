...
class tool {
  public:
    ...
    static std::vector<std::string> splitStr(const std::string &, std::string);
};
...
class kitchen {
  private:
    pos oven, blueberry, icecream, strawberry, windows, dish, choppingBoard, dough;
...
class chef {
  ...
  public:
    ...
    player me, partner;
    std::string recipe;
    std::vector<std::string> chopList, ovenList, mergeList, lackList;
...
std::vector<std::string> tool::splitStr(const std::string &str, std::string del) {
...
void kitchen::initKitchen() {
    for (int i = 0; i < mazeYNum; ++i) {
        for (int j = 0; j < mazeXNum; ++j) {
            if(maze[i][j] == 'B') blueberry = {j, i};
            else if(maze[i][j] == 'I') icecream = {j, i};
            else if(maze[i][j] == 'S') strawberry = {j, i};
            else if(maze[i][j] == 'D') dish = {j, i};
            else if(maze[i][j] == 'W') windows = {j, i};
            else if(maze[i][j] == 'C') choppingBoard = {j, i};
            else if (maze[i][j] == 'H') dough = {j, i};
            else if (maze[i][j] == 'O') oven = {j, i};
        }
    }
}

pos kitchen::getItemPos(const std::string & name) const {
    if(name == "BLUEBERRIES")            return blueberry;
    else if(name == "ICE_CREAM")        return icecream;
    else if(name == "STRAWBERRIES")      return strawberry;
    else if(name == "DISH")            return dish;
    else if(name == "WINDOW")         return windows;
    else if(name == "CHOPPED")   return choppingBoard;
    else if(name == "DOUGH")           return dough;
    else if(name == "OVEN")            return oven;
    else {
        auto it = std::find_if(table.begin(), table.end(),
        [&](const std::pair<pos, std::string>& element) {
            return element.second == name;
        });
        return it == table.end() ? pos(-1, -1) :
               it->first;
    }
}
...
bool kitchen::isEmptyTable(const pos &inPos) const {
    if(inPos.x < 0 || inPos.x > 10 || inPos.y < 0 || inPos.y > 6) return false;
    if(maze[inPos.y][inPos.x] != '#') return false;
    for(auto &it : table) {
        if(inPos == it.first) return false;
    }
    return true;
}

std::vector<std::pair<pos, std::string>> kitchen::getTableItem() const {
    return table;
}

void customer::addOrder(const std::string &item, int award) {
    order.emplace_back(item, award);
    sort(order.begin(), order.end(), [](const std::pair<std::string, int> &a, const std::pair<std::string, int> &b) {
...
std::vector<std::pair<std::string, int>> customer::getOrder() const {
    return order;
}
...
void chef::roundInit() {
    ...
    for (int i = 0; i < numCustomers; ++i) {
        ...
        int customerAward;
        std::cin >> customerItem >> customerAward;
        if(recipe == customerItem) recipeFlag = true;
        client.addOrder(customerItem, customerAward);
    }
    if(!recipeFlag) {
        recipe.clear();
...
bool chef::action(const std::string &mode, const pos & destination) const {
    std::cout << mode << " " << destination << std::endl;
    if(mode == "USE") {
        if(abs(me.getPos().x - destination.x) <= 1 && abs(me.getPos().y - destination.y) <= 1)
            return true;
    }
    return false;
}
...
void chef::recipeStratege(const std::string &type) {
    ...
    else if(type == "MaxAward") retPos = 0;
    else if(type == "SecondAward") retPos = 1;
    else if(type == "MinAward") retPos = 2;
...
void chef::checkLackList() {
    ...
    for(auto &it : meHandList) {
        if(std::find(recipeList.begin(), recipeList.end(), it) == recipeList.end()) {
    ...
    if(flagHandCanUse) {
        for(auto &it : meHandList) {
            if(auto findPos = std::find(lackList.begin(), lackList.end(), it); findPos != lackList.end())
                lackList.erase(findPos);
        }
    }
    bool flagTableCanUse = false;
    ...
    for(auto &i : tableItem) {
        ...
        if(i.second.find("DISH") != std::string::npos) {
            ...
            for(auto &j : targetTableItemList) {
                if(std::find(recipeList.begin(), recipeList.end(), j) == recipeList.end()) {
                    flagTableCanUse = false;
                    break;
                }
            }
        }
        if(flagTableCanUse) break;
    }
    if(flagTableCanUse) {
...
bool chef::foodDelivery() {
    if(!lackList.empty()) {
        ...
        for(auto &it : lackList) {
            if(it == "CHOPPED_STRAWBERRIES" && std::find(chopList.begin(), chopList.end(), "STRAWBERRIES") == chopList.end())
                chopList.emplace_back("STRAWBERRIES");
            else if(it == "CROISSANT" && std::find(ovenList.begin(), ovenList.end(), "DOUGH") == ovenList.end())
                ovenList.emplace_back("DOUGH");
            else if(it == "TART") {
                if(cookhouse.getItemPos("RAW_TART") == pos(-1, -1) && me.getHand() != "RAW_TART") {
                    if(cookhouse.getItemPos("CHOPPED_DOUGH") == pos(-1, -1) && me.getHand() != "CHOPPED_DOUGH") {
                        if(std::find(chopList.begin(), chopList.end(), "DOUGH") == chopList.end())
                            chopList.emplace_back("DOUGH");
                    }
                }
                if(std::find(ovenList.begin(), ovenList.end(), "RAW_TART") == ovenList.end())
    ...
    if(meHand != "NONE" && meHand.find("DISH") == std::string::npos) {
        put();
        return true;
    }
    auto recipeList = tool::splitStr(recipe, "-");
    ...
    auto meHandList = tool::splitStr(me.getHand(), "-");
    for(auto &it : meHandList) {
        if(auto findPos = std::find(recipeList.begin(), recipeList.end(), it); findPos != recipeList.end())
            recipeList.erase(findPos);
    }
    if(!recipeList.empty()) {
        pos dishPosXY = cookhouse.getItemPos(recipeList[0]);
        if(action("USE", dishPosXY) && recipeList[0] == "WINDOW") {
            clearAll();
            chefState = ALLDONE;
        }
        return true;
...
void chef::put() const {
    for(int i = 0; i < 8; ++i) {
        pos nPos = {me.getPos().x + tool::d[i][0], me.getPos().y + tool::d[i][1]};
...
void chef::chop() {
    ...
    std::string tmpItem = (meItem == chopList[0]) ? "CHOPPED" : chopList[0];
...
void chef::oven() {
    ...
    std::string tmpItem = (meItem == ovenList[0]) ? "OVEN" : ovenList[0];
    pos dishPosXY = cookhouse.getItemPos(tmpItem);
